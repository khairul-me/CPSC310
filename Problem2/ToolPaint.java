...
public class ToolPaint extends Application {
    ...
    private PrintWriter coordOutput; // For writing coordinates to file
    ...
    private Color currentColor = Color.RED;  // The current drawing color.
    ...
    /////////////////////////////
    private int currentImageIndex = 1; //Khairul- Start with first image
    private Label imageNameLabel;  // To display current image name
    ...
    public void start(Stage stage) {
        ...
        try {
            coordOutput = new PrintWriter(new FileWriter("brain_coordinates.txt"));
            fileInitialized = true;
            System.out.println("Coordinate output file created successfully");
        } catch (IOException e) {
            System.out.println("Error creating coordinate file: " + e.getMessage());
        }
        ...
        imageNameLabel = new Label("Screenshot_17-2-2025_22171_www.imaios.com.jpeg");

        prevButton.setOnAction(e -> showPreviousImage());
        nextButton.setOnAction(e -> showNextImage());

        bottomPanel.getChildren().addAll(prevButton, imageNameLabel, nextButton);
        root.setBottom(bottomPanel); // Add the navigation panel
    ...
    private void mousePressed(MouseEvent evt) {
        ...
        // Add this section to save coordinates to file when using LINE tool
        if (currentTool == Tool.LINE && fileInitialized) {
            try {
                // Write coordinates to file in format: imageIndex x y
                coordOutput.println(currentImageIndex + " " + currentX + " " + currentY);
                coordOutput.flush(); // Ensure data is written immediately
                System.out.println("Saved coordinate: Image " + currentImageIndex + 
                                " at position (" + currentX + "," + currentY + ")");
            } catch (Exception e) {
                System.out.println("Error writing coordinates: " + e.getMessage());
            }
        }
        if (currentTool == Tool.ERASE) {
    ...
    private void showPreviousImage() {
        if (currentImageIndex > 1) {
            if (fileInitialized) {
                // Add a separator for the previous image
                coordOutput.println("# New image " + (currentImageIndex - 1));
                coordOutput.flush();
            }
            currentImageIndex--;
            loadImage(currentImageIndex);
    ...
    private void showNextImage() {
        if (currentImageIndex < 33) {  // There are 33 images total
            if (fileInitialized) {
                // Add a separator for the next image
                coordOutput.println("# New image " + (currentImageIndex + 1));
                coordOutput.flush();
            }
            currentImageIndex++;
    ...
    private void loadImage(int index) {
        // Array of image numbers (extracted from your filenames)
        int[] imageNumbers = {
            22171, 22180, 22190, 22210, 22225, 221545, 221632, 221645, 221659, 221710, 221721, 221722,
            221735, 221748, 221811, 221821, 221831, 221838, 221840, 221912, 221913, 221956, 222011, 222026,
            222048, 222111, 222113, 222123, 222134, 222145, 222156, 222216, 222226
        };
        
        // Adjust index to array bounds (1-based to 0-based)
        int arrayIndex = index - 1;
        if (arrayIndex < 0 || arrayIndex >= imageNumbers.length) {
            System.out.println("Invalid image index: " + index);
            return;
        }
        
        // Important: Use .jpeg extension instead of .png
        String filename = String.format("Screenshot_17-2-2025_%d_www.imaios.com.jpeg", imageNumbers[arrayIndex]);
        
        // Use forward slashes for paths
        String fullPath = "C:/Users/khair/OneDrive/Documents/CPSC 310/HW 02/Problem2/images/" + filename;
        System.out.println("Trying to load: " + fullPath);
        
        try {
            File file = new File(fullPath);
            if (file.exists()) {
                System.out.println("File exists at: " + fullPath);
                // Use forward slashes for the file: URL format
                Image image = new Image("file:" + fullPath);
                canvasGraphics.clearRect(0, 0, canvas.getWidth(), canvas.getHeight());
                canvasGraphics.drawImage(image, 0, 0, canvas.getWidth(), canvas.getHeight());
                imageNameLabel.setText("File: " + filename);
            } else {
...
@Override
public void stop() {
    if (fileInitialized && coordOutput != null) {
        try {
            coordOutput.flush();
            coordOutput.close();
            System.out.println("Coordinate file closed successfully");
        } catch (Exception e) {
            System.out.println("Error closing coordinate file: " + e.getMessage());
        }
    }
}
